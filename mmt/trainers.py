...
class MultiTrainer_2scale(object):
    def __init__(self, model, num_classes, margin=0.0):
        super(MultiTrainer_2scale, self).__init__()
        self.model = model
    ...
    def train(self, epoch, data_loader_source, data_loader_target, optimizer, train_iters=200, print_freq=1):
        ...
        for i in range(train_iters):
            ...
            t_features = self.model(t_inputs)[0]
            s_features = outputs[1:3]
            s_cls_out = outputs[3:]
            # backward main #
            loss_ce, loss_tr, prec1 = self._forward(s_features, s_cls_out, targets)
            loss_ce=loss_ce/len(s_cls_out)
            loss_tr=loss_tr/len(s_features)
    ...
    def _forward(self, s_features, s_outputs, targets):
        ...
        for ll in range(len(s_outputs)):
          loss_ce += self.criterion_ce(s_outputs[ll], targets)
        
        for ll in range(len(s_features)):
            loss_tr += self.criterion_triple(s_features[ll], s_features[ll], targets)
        prec, = accuracy(s_outputs[0].data, targets.data)
        prec = prec[0]
